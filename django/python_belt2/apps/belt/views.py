...
def edit_account(request, user_id):
    ...
    return render(request, 'belt/edit_account.html', context)
...
def comment(request):
    ...
    valid = True
    if len(f['comment']) < 10:
        messages.error(request, "Comment must be at least 10 characters.")
        valid = False
    ...
    if not valid:
        ...
        c= Comment()
        c.comment=f['comment']
        c.commented_by=User.objects.get(id=request.session['user_id'])
        c.commented_on=Quote.objects.get(id=f['quote_id'])
        # this_comment = Comment.objects.get(id=comment_id)
        # this_comment.commented_by.add(this_quote)
        c.save()
